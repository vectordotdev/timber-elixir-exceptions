defmodule Timber.Exceptions.ErrorLogger do
  @moduledoc """
  Handles error reports from the `:error_logger` application.

  Timber can automatically log the exceptions that occur in your application as
  exception events with all the necessary metadata by registering as an
  `:error_logger` handler. To activate Timber's error logging system, you need
  to add a few configuration lines and add the `ErrorLogger` handler during
  application start:

  ```
  # Enable Timber's error capturing system
  config :timber_exceptions, :capture_errors, true
  ...
  def start(_type, _opts) do
    ...
    :ok = :error_logger.add_report_handler(Timber.Exceptions.ErrorLogger)

    Supervisor.start_link(children, opts)
  end
  ```

  Users should be aware that report handlers can be added multiple times, which
  can result in duplicated logs. This concern is most relevant to umbrella
  applications or applications with multiple entry-points. To solve this, the
  handler should be added only at the primary application entry-point. If that
  is not feasible, the other solution is to check if the handler exists before
  adding it. Example:

  ```
  if !(Timber.Exceptions.ErrorLogger in :gen_event.which_handlers(:error_logger)) do
    :ok = :error_logger.add_report_handler(Timber.Exceptions.ErrorLogger)
  end
  ```
